...
for(const token of ['championship-fixture-ready','openFixture','toggleVoice','game-voice-ready','game-voice-state','activeFixtureId','showQr','shareAccess','joinRoomCode'])if(!js.includes(token))throw new Error(`Fluxo ausente: ${token}`);
const server=fs.readFileSync(path.join(root,'server.js'),'utf8');
for(const token of ['ensureFixtureRoom','notifyFixtureReady','sendFixtureReadyToPlayer','currentFixtureForPlayer','playerSockets','championships/:id/start','championships/:id/fixtures/:fixtureId/room','championships/:id/restart','game-voice-ready','game-voice-state','chatAllowed','/vendor/qrcode.min.js'])if(!server.includes(token))throw new Error(`Servidor incompleto: ${token}`);
...
if(!server.includes("const APP_VERSION = '2.3.0';"))throw new Error('Versão do servidor não foi atualizada');
