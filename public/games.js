...
let pc=null,localStream=null,voiceOn=false,isSpectator=false,currentChampChat=[],pendingIce=[],voiceNegotiating=false,activeFixtureId=null,resultActionToken=0,lastChatSentAt=0;
const seen=new Set();
...
function addMsg(name,text,me,scope,at){const n=document.createElement('div');n.className='msg'+(me?' me':'');const body=document.createElement('span');body.textContent=text;const who=document.createElement('small');who.textContent=(scope==='team'?'Equipa · ':'')+(name||'Jogador')+(at?' · '+timeLabel(at):'');n.append(body,who);$('messages').appendChild(n);$('messages').scrollTop=$('messages').scrollHeight}
function sendChat(){const input=$('chatInput'),text=input.value.trim();if(!text)return;if(Date.now()-lastChatSentAt<500)return toast('Aguarda um instante.');if(chatScope==='team'&&isSpectator)return toast('👀 O modo espectador não pode usar o chat da equipa.');const clientMessageId=crypto.randomUUID();const payload={type:'game-chat',text,scope:chatScope,clientMessageId};if(send(payload)){input.value='';lastChatSentAt=Date.now()}}
...
$('backChamp').onclick=async()=>{const cid=state?.championshipFixture?.championshipId;if(cid){page('championships');await openChampionship(cid)}};$('quickRoom').onclick=create;$('joinBtn').onclick=join;$('copyCode').onclick=async()=>{try{await navigator.clipboard.writeText(roomCode);toast('Código copiado.')}catch{toast('Código: '+roomCode)}};$('shareCode').onclick=shareRoomCode;$('leaveGame').onclick=leave;$('rematchBtn').onclick=()=>{if(!state?.winner)return;if(send({type:'game-reset'}))toast('Revanche iniciada.');};$('voiceBtn').onclick=toggleVoice;$('chatSend').onclick=sendChat;$('chatInput').onkeydown=e=>{if(e.key==='Enter')sendChat()};document.querySelectorAll('.chat-tabs button').forEach(b=>b.onclick=()=>{chatScope=b.dataset.scope;document.querySelectorAll('.chat-tabs button').forEach(x=>x.classList.toggle('active',x===b));$('messages').innerHTML='';if(chatScope==='team'&&!state?.teams?.[mySymbol])toast('Ainda não estás associado a uma equipa.')});document.querySelectorAll('nav button').forEach(b=>b.onclick=()=>page(b.dataset.page));$('createCup').onclick=()=>{$('cupCreate').hidden=false;$('cupJoin').hidden=true;$('cupAccess').hidden=true};$('cupCancel').onclick=()=>{$('cupCreate').hidden=true};$('joinCup').onclick=()=>{$('cupJoin').hidden=false;$('cupCreate').hidden=true;$('cupAccess').hidden=true;$('cupJoinName').value=myName};$('cupJoinCancel').onclick=()=>{$('cupJoin').hidden=true};$('cupCreateSubmit').onclick=createChampionship;$('cupJoinSubmit').onclick=joinChampionship;$('cupChatSend').onclick=sendChampionshipChat;$('cupChatInput').onkeydown=e=>{if(e.key==='Enter')sendChampionshipChat()};$('qrClose').onclick=closeQr;$('qrModal').onclick=e=>{if(e.target===$('qrModal'))closeQr()};$('qrShare').onclick=()=>{const url=$('qrLink').href;navigator.share?navigator.share({title:$('qrTitle').textContent,url}).catch(()=>{}):navigator.clipboard?.writeText(url).then(()=>toast('🔗 Link copiado.'))};$('qrDownload').onclick=()=>{const canvas=$('qrCanvas');if(!canvas.hidden){const a=document.createElement('a');a.download='2-on-acesso-qr.png';a.href=canvas.toDataURL('image/png');a.click()}else window.open($('qrImage').src,'_blank','noopener');};
