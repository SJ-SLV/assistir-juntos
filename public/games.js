...
function renderChat(){const root=$('messages');if(!root)return;root.innerHTML='';if(chatScope==='general'){globalMessages.forEach(m=>addMsg(m.name,m.text,m.playerId===playerId,'general',m.at));globalUnread=0}else{teamMessages.forEach(m=>addMsg(m.name,m.text,m.playerId===playerId,'team',m.at));teamUnread=0}updateTeamBadge()}
function renderGlobalMessages(){const root=$('globalMessages');if(!root)return;root.innerHTML='';globalMessages.forEach(m=>addMsg(m.name,m.text,m.playerId===playerId,'general',m.at,'globalMessages'))}
...
function sendGlobalChat(){const input=$('globalChatInput'),text=input?.value.trim();if(!text)return;if(Date.now()-lastChatSentAt<500)return toast('Aguarda um instante.');if(send({type:'global-chat',text,playerId,clientMessageId:crypto.randomUUID()})){input.value='';lastChatSentAt=Date.now()}}
...
async function showQr(title,code,kind='join'){
  ...
  canvas.hidden=true;fallback.hidden=false;fallback.src='https://api.qrserver.com/v1/create-qr-code/?size=240x240&data='+encodeURIComponent(url);modal.classList.add('open');modal.setAttribute('aria-hidden','false');
}
...
function renderChampionship(c){
  ...
  currentChampChat=c.chat||[];teamMessages=currentChampChat.filter(m=>m.scope==='team');if($('cupMessages')){$('cupMessages').innerHTML='';currentChampChat.forEach(m=>addChampMsg(m.name,m.text,m.playerId===playerId,m.scope||'general',m.at))}
...
async function fetchChampionshipSilently(id){
  try{
    const r=await fetch('/api/games/championships/'+encodeURIComponent(id)+'?playerId='+encodeURIComponent(playerId),{cache:'no-store'});
    const d=await r.json();
...
window.goToHomePanel=()=>page('home');
$('backChamp').onclick=async()=>{const cid=state?.championshipFixture?.championshipId;if(cid){page('championships');await openChampionship(cid)}};
function openLobbyEntry(mode='create'){
  const box=$('lobbyEntry'); if(!box)return;
  box.hidden=false;
  if(mode==='create') $('lobbyCreateBtn').focus(); else $('lobbyJoinBtn').focus();
  box.scrollIntoView({behavior:'smooth',block:'nearest'});
}
$('openGameEntry').onclick=()=>openLobbyEntry('create');
...
document.querySelectorAll('.lobby-game-button').forEach(b=>b.onclick=()=>{
  ...
  if(game==='tictactoe') openLobbyEntry('create');
  else toast('🎮 Este jogo está em preparação. O motor multiplayer será ligado nesta área quando a implementação do jogo estiver concluída.');
});
...
function joinRoomCode(code){if(!startWithData())return;roomCode=code;mySymbol=null;activeFixtureId=null;const action={type:'game-join',roomCode:code,name:myName,teamName:myTeam,playerId};pendingAction=action;if(!socketOpen()){toast('🔄 A ligar à partida…');connect();return}pendingAction=null;send(action)}
...
window.addEventListener('beforeunload',()=>{intentionalClose=true;stopVoice()});connect();handleDeepLink();
})();
